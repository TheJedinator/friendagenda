...
$result = mysqli_query($con, "SELECT * FROM friends WHERE  friends_with = '$id' and  member_id!= '$id' and status = 'conf'    OR member_id = '$id' and friends_with != '$id' and status = 'conf' ");
  ...
  <?php
		...
		//check to see if any rows were actually retrieved
...
<div class="right">
	...
	<div class="bkb"></div>
		...
		<?php
			$id = $_SESSION['SESS_MEMBER_ID'];
			$strSQL = "SELECT * FROM members WHERE member_id != '$id' LIMIT 0,3";
			$post = mysqli_query($con, $strSQL)or die(mysql_error());
			while($row = mysqli_fetch_array($post)){
				echo '
				<ul id="sddm11">
				<li>
...
<hr align="left" width="420">
	   <?php
echo '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
$result = mysqli_query($con, "SELECT * FROM photos WHERE member_id='".$_SESSION['SESS_MEMBER_ID'] ."'  ORDER BY photo_id DESC LIMIT 0,4");
...
<div class="shoutout">
		...
		<div  class="back"><h4><class="p"><div></h4></div>
          ...
          <input name="memberid" type="hidden" id="memberid" value="<?php if (isset($_GET['id'])){ echo $_GET['id']; }else { echo "";}  ?>"/>
