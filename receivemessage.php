...
$member_id = $_SESSION['SESS_MEMBER_ID'];
$received = mysqli_query($con, "SELECT * FROM messages WHERE recipient = '$member_id'")or die(mysql_error());
...
$result = mysqli_query($con, "SELECT * FROM friends WHERE status='conf'");
  ...
  <?php					
		$member_id=$_SESSION['SESS_MEMBER_ID'];							
		$post = mysqli_query($con, "SELECT * FROM friends WHERE  status = 'conf' ")or die(mysql_error());		
...
<?php 			
		$member_id = $_SESSION['SESS_MEMBER_ID'];
		$post = mysqli_query($con, "SELECT * FROM messages WHERE recipient = '$id' ORDER by datetime DESC")or die(mysql_error());
		while($row = mysqli_fetch_array($post)){
			$id = $row['receiver'];
			$hu_u = mysqli_query($con, "SELECT * FROM members WHERE member_id = '$id'")or die(mysql_error());
			$rows = mysqli_fetch_array($hu_u);
			$iyaid = $row['message_id'];
			echo'<div class="information">';
			echo'<hr width=640>
			</br>';
				$sql=mysqli_query($con, "SELECT * FROM members WHERE member_id='$id'") or die(mysql_error());
