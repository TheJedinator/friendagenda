...
$member_id = $_SESSION['SESS_MEMBER_ID'];
$received = mysqli_query($con, "SELECT * FROM messages WHERE receiver = '$member_id'")or die(mysql_error());
...
<div><form name="" method="post">
	...
	<?php
		$member_id = $_SESSION['SESS_MEMBER_ID'];
		$post = mysqli_query($con, "SELECT * FROM members WHERE member_id = '$member_id'")or die(mysql_error());
	...
	<?php 
		$member_id=$_SESSION['SESS_MEMBER_ID'];							
		$post = mysqli_query($con, "SELECT * FROM friends WHERE friends_with = '$member_id' OR member_id = '$member_id' AND status = 'conf' ")or die(mysql_error());
	...
	<br />
		...
		mysqli_query($con, "INSERT INTO messages (receiver, recipient, datetime, content, status)
